...
Q=q^2eye(n); % covariance of process
...
zV = zeros(1,N);
for k=1:N
z = h(s) + rrandn; % measurments
sV(:,k)= s; % save actual state
zV(k) = z; % save measurment
[x, P] = ukf(f,x,P,h,z,Q,R); % ekf
xV(:,k) = x; % save estimate
s = f(s) + qrandn(3,1); % update process
end
...
P12=X2diag(Wc)*Z2'; %transformed cross-covariance
K=P12inv(P2);
x=x1+K(z-z1); %state update
P=P1-KP12'; %covariance update
